...
:root {
  ...
  --color-background-primary: #0F0A2E;
  --color-background-secondary: #1A1245;
  --color-background-tertiary: #1A1245;
  --color-text-primary: #FFFFFF;
  --color-text-secondary: #AFA9EC;
  --color-text-tertiary: #7B72D4;
...
.badge-gray{background:rgba(255,255,255,.1);color:var(--text3)}
...
/* ===== SCREEN BLACKOUT PREVENTION ===== */
*{-webkit-tap-highlight-color:transparent}
html{-webkit-text-size-adjust:100%}
body{-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale}

/* ===== RESPONSIVE — ALL LAPTOPS ===== */
/* Large screens 1400+ */
@media(min-width:1400px){
  .main-content{padding:32px 40px}
  .card{padding:24px}
}
/* Standard laptops 1024-1400 */
@media(max-width:1399px) and (min-width:1024px){
  .main-content{padding:24px 28px}
}
/* Small laptops 768-1024 */
@media(max-width:1024px) and (min-width:768px){
  .sidebar{width:180px;min-width:180px}
  .main-content{margin-left:180px;padding:18px 20px}
...
@media(max-width:400px){
  .main-content{padding:10px 10px 75px}
  .stats-grid{grid-template-columns:1fr 1fr}
  .card{padding:12px}
}
</style>
...
<!-- MOBILE BOTTOM NAV -->
<div style="display:none;position:fixed;bottom:0;left:0;right:0;background:var(--bg2);padding:10px 20px;justify-content:space-around;align-items:center;z-index:1000;border-top:0.5px solid rgba(255,255,255,.1)" class="mobile-bottom-nav">
  {mob_nav}
</div>
<style>
@media(max-width:768px){{
...
def new_qc():
    user = get_current_user()
    return render_template_string(SHARED_CSS + f"""
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=5.0"><title>New QC — SurveyQC</title></head><body>
<div class="app-layout">
  {sidebar_html('new-qc')}
  <div class="main-content">
    <div class="topbar">
    ...
    <div style="display:grid;grid-template-columns:1fr 220px;gap:16px">
      <form action="/run-qc" method="POST" enctype="multipart/form-data">
        <div class="card" style="margin-bottom:16px">
          <p style="font-size:14px;font-weight:600;color:var(--text);margin-bottom:16px">Upload files</p>
        ...
        <div class="card" style="margin-bottom:16px">
          <p style="font-size:13px;font-weight:600;color:var(--text);margin-bottom:14px">Checks to run</p>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ...
      <div style="display:flex;flex-direction:column;gap:12px">
        ...
        <div class="card-purple">
          ...
          <div style="display:flex;flex-direction:column;gap:6px">
            <div style="display:flex;align-items:center;gap:8px;padding:7px 10px;background:var(--bg3);border-radius:7px"><span class="dot dot-purple"></span><span style="font-size:12px;color:var(--text2)">Confirmit</span></div>
            <div style="display:flex;align-items:center;gap:8px;padding:7px 10px;background:var(--bg3);border-radius:7px"><span class="dot dot-amber"></span><span style="font-size:12px;color:var(--text2)">Decipher</span></div>
            <div style="display:flex;align-items:center;gap:8px;padding:7px 10px;background:var(--bg3);border-radius:7px"><span class="dot dot-green"></span><span style="font-size:12px;color:var(--text2)">Forsta</span></div>
            <div style="display:flex;align-items:center;gap:8px;padding:7px 10px;background:var(--bg3);border-radius:7px"><span class="dot dot-red"></span><span style="font-size:12px;color:var(--text2)">Qualtrics</span></div>
          </div>
        </div>
      </div>
    </div>
...
def ai_tester():
    return render_template_string(SHARED_CSS + f"""
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=5.0"><title>AI Tester — SurveyQC</title></head><body>
<div class="app-layout">
  ...
  <div class="main-content">
    ...
    <div style="display:grid;grid-template-columns:1fr 260px;gap:16px">
      <div class="card">
        <p style="font-size:14px;font-weight:600;color:var(--text);margin-bottom:14px">How AI Tester works</p>
...
<div style="display:flex;min-height:100vh">
  <div style="width:220px;min-width:220px;background:var(--bg2);border-right:0.5px solid var(--border);padding:20px 12px;position:fixed;height:100vh;overflow-y:auto">
...
def admin_content():
    ...
    return render_template_string(SHARED_CSS + f"""
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=5.0"><title>Content — Admin</title></head><body>
...
def home_landing():
    ...
    return render_template_string(SHARED_CSS + f"""
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=5.0"><title>{content['site_name']} — AI Survey QC</title>
<style>
.pub-nav{{background:var(--bg2);padding:13px 40px;display:flex;align-items:center;justify-content:space-between}}
...
<div class="pub-hero">
  ...
  <div style="display:flex;gap:12px;justify-content:center;margin-bottom:12px">
    <a href="/signup" style="background:var(--purple);color:white;font-size:14px;padding:13px 30px;border-radius:9px;font-weight:500;text-decoration:none">{content['hero_cta']}</a>
...
<div class="how-grid">
  <div style="background:var(--color-background-primary);border:0.5px solid var(--color-border-tertiary);border-radius:12px;padding:20px;text-align:center">
    <div style="width:38px;height:38px;border-radius:50%;background:var(--purple);color:white;font-size:16px;font-weight:600;display:flex;align-items:center;justify-content:center;margin:0 auto 12px">1</div>
  ...
  <div style="background:var(--color-background-primary);border:0.5px solid var(--color-border-tertiary);border-radius:12px;padding:20px;text-align:center">
    <div style="width:38px;height:38px;border-radius:50%;background:var(--purple);color:white;font-size:16px;font-weight:600;display:flex;align-items:center;justify-content:center;margin:0 auto 12px">2</div>
  ...
  <div style="background:var(--color-background-primary);border:0.5px solid var(--color-border-tertiary);border-radius:12px;padding:20px;text-align:center">
    <div style="width:38px;height:38px;border-radius:50%;background:var(--purple);color:white;font-size:16px;font-weight:600;display:flex;align-items:center;justify-content:center;margin:0 auto 12px">3</div>
...
<div class="price-grid">
  ...
  <div class="price-card featured" style="position:relative">
    ...
    <a href="/signup" style="display:block;text-align:center;background:var(--purple);color:white;padding:10px;border-radius:8px;font-size:13px;font-weight:500;text-decoration:none;margin-top:16px">Start Pro trial</a>
  </div>
  <div class="price-card">
...
<!-- CTA -->
<div style="background:var(--bg2);padding:50px 20px;text-align:center">
  ...
  <a href="/signup" style="background:var(--bg2);color:#042C53;font-size:14px;padding:13px 32px;border-radius:9px;font-weight:600;text-decoration:none">Start free — no card required</a>
</div>
...
def admin_apis():
    ...
    return render_template_string(SHARED_CSS + f"""
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=5.0"><title>API Management — Admin</title></head><body>
<div style="padding:24px">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
    ...
    <div style="display:flex;gap:8px">
      ...
      <div style="background:var(--bg3);border-radius:8px;padding:8px 14px;text-align:center">
