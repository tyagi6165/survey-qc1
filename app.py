...
:root {
  ...
  --blue: #378ADD;
  --purple: #534AB7;
  --purple-dim: rgba(83,74,183,0.08);
...
.sidebar{width:220px;min-width:220px;background:#042C53;border-right:none;padding:20px 12px;display:flex;flex-direction:column;gap:4px;position:fixed;height:100vh;overflow-y:auto}
.main-content{margin-left:220px;flex:1;padding:28px;min-height:100vh;background:#F0F2F5}
.sidebar-logo{display:flex;align-items:center;gap:9px;padding:0 8px;margin-bottom:22px}
.sidebar-logo-icon{width:30px;height:30px;background:#378ADD;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:15px;flex-shrink:0}
.sidebar-logo-text{color:white;font-size:15px;font-weight:500}
.nav-item{display:flex;align-items:center;gap:10px;padding:9px 12px;border-radius:8px;cursor:pointer;text-decoration:none;color:#85B7EB;font-size:13px;transition:all .15s}
.nav-item:hover{background:rgba(255,255,255,.08);color:white}
.nav-item.active{background:rgba(255,255,255,.15);border-left:2px solid #378ADD;color:white;padding-left:10px}
.nav-item i{font-size:16px;width:18px;text-align:center}
.nav-divider{border-top:0.5px solid rgba(255,255,255,.1);margin:8px 0}
.nav-section{font-size:10px;color:rgba(255,255,255,.4);letter-spacing:.08em;text-transform:uppercase;padding:4px 12px;margin-top:4px}
...
.card{background:white;border:0.5px solid #DDE1E7;border-radius:var(--card-r);padding:20px}
.card-sm{background:white;border:0.5px solid #DDE1E7;border-radius:var(--card-r);padding:14px}
.card-purple{background:#E6F1FB;border:0.5px solid #B5D4F4;border-radius:var(--card-r);padding:16px}
.card-hero{background:#042C53;border-radius:var(--card-r);padding:22px}
.stats-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;margin-bottom:14px}
.stat-card{background:white;border:0.5px solid #DDE1E7;border-radius:var(--card-r);padding:16px;text-align:center}
.stat-num{font-size:22px;font-weight:500;color:#1A1A2E}
.stat-label{font-size:11px;color:#6B7280;margin-top:4px}
.stat-change{font-size:10px;margin-top:2px}
.stat-up{color:#27500A}
.stat-down{color:#791F1F}
...
.btn{display:inline-flex;align-items:center;justify-content:center;gap:6px;padding:8px 16px;border-radius:8px;font-size:13px;font-weight:500;cursor:pointer;border:none;transition:all .15s;text-decoration:none}
.btn-primary{background:#042C53;color:white}
.btn-primary:hover{background:#053468}
.btn-secondary{background:#378ADD;color:white}
.btn-ghost{background:transparent;color:#374151;border:0.5px solid #DDE1E7}
.btn-ghost:hover{background:#F8F9FA}
.btn-danger{background:#FCEBEB;color:#791F1F}
.btn-sm{padding:5px 12px;font-size:12px;border-radius:7px}
.form-group{margin-bottom:14px}
.form-label{font-size:12px;font-weight:500;color:#374151;display:block;margin-bottom:5px}
.form-input{width:100%;padding:9px 12px;border:0.5px solid #DDE1E7;border-radius:8px;font-size:13px;color:#1A1A2E;background:white;outline:none}
.form-input:focus{border-color:#378ADD;box-shadow:0 0 0 3px rgba(55,138,221,.1)}
textarea.form-input{resize:vertical;min-height:100px}
.table-wrap{overflow-x:auto}
table{width:100%;border-collapse:collapse}
th{padding:10px 12px;text-align:left;font-size:10px;color:#6B7280;font-weight:500;border-bottom:0.5px solid #DDE1E7;background:#F8F9FA;text-transform:uppercase;letter-spacing:.05em}
td{padding:11px 12px;font-size:13px;color:#374151;border-bottom:0.5px solid #F0F2F5}
tr:hover td{background:#FAFAFA}
.mobile-nav{display:none;position:fixed;bottom:0;left:0;right:0;background:white;border-top:0.5px solid #DDE1E7;z-index:200;padding:6px 0}
.mobile-nav-inner{display:flex;justify-content:space-around}
.mobile-nav-item{display:flex;flex-direction:column;align-items:center;gap:3px;padding:6px 12px;text-decoration:none;color:#9CA3AF;font-size:10px;border-radius:8px}
.mobile-nav-item.active{color:#042C53}
.mobile-nav-item i{font-size:20px}
@media(max-width:768px){
.sidebar{display:none}
.main-content{margin-left:0;padding:16px;padding-bottom:70px}
...
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=5.0"><title>Login — SurveyQC</title></head><body>
<div class="auth-page">
  <div class="auth-left">
  ...
  <div class="auth-right">
    <h2 style="font-size:22px;font-weight:600;color:var(--text);margin-bottom:6px">Welcome back</h2>
...
def signup():
    ...
    return render_template_string(SHARED_CSS + f"""
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=5.0"><title>Sign Up — SurveyQC</title></head><body>
<div class="auth-page">
  <div class="auth-left">
    <div style="margin-bottom:32px">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:28px">
  ...
  <div class="auth-right">
    <h2 style="font-size:22px;font-weight:600;color:var(--text);margin-bottom:6px">Create your account</h2>
...
site_content = {
    ...
    'hero_subheading': 'Upload spec doc + paste survey URL. AI tests every path, catches every bug, works in any language.',
    ...
    'feature2_desc': 'Catches every missing word between doc and live survey',
    'feature3_title': 'Auto Screenshots',
    'feature3_desc': 'Every bug captured with screenshot proof automatically',
    'feature4_title': 'Any Language',
    'feature4_desc': 'French, Italian, Urdu, English -- 80+ languages supported',
    'feature5_title': '8 QC Checks',
    'feature5_desc': 'Termination, text, words, piping, codes and more',
    'feature6_title': 'One-click Retest',
    'feature6_desc': 'Fix and retest only the failed paths instantly',
    ...
    'site_name': 'SurveyQC',
    'tagline': 'The world first AI-powered survey QC tool',
    ...
    'blog_post2_title': '8 Most Common Survey Bugs (And How to Catch Them Automatically)',
    'blog_post2_date': 'April 2026',
    'blog_post2_tag': 'Best Practices',
    'blog_post2_summary': 'Termination errors, missing words, broken piping -- the 8 bugs that destroy survey data and how SurveyQC catches all of them.',
    'blog_post3_title': 'French Survey QC: Why Accents and Encoding Matter',
    'blog_post3_date': 'March 2026',
    'blog_post3_tag': 'Languages',
    'blog_post3_summary': 'Testing French surveys has unique challenges -- accent marks, special characters, and termination phrases. Our full approach.',
    ...
    'blog_post5_summary': 'Most teams still use Excel for survey QC. We analyzed 500 surveys to show why that is costing you 40+ hours per month.',
    ...
    'compare_chatgpt_summary': 'ChatGPT is a general AI. SurveyQC is purpose-built for survey quality control with 8 specialized checks, Playwright crawling, and Word report output.',
    'compare_excel_heading': 'SurveyQC vs Manual Excel QC',
    'compare_excel_summary': 'Excel takes 8+ hours per survey. SurveyQC does it in 10 minutes automatically with screenshots and a professional report.',
    'compare_manual_heading': 'SurveyQC vs Manual Testing',
    'compare_manual_summary': 'Manual testers miss 30% of bugs on average. SurveyQC catches 99% automatically -- including edge cases humans always skip.',
    'community_heading': 'Join the SurveyQC Community',
    'community_subheading': 'Connect with 500+ QC professionals worldwide. Share tips, get help, and improve your QC workflow.',
    'affiliate_heading': 'Earn with SurveyQC Affiliate Program',
    'affiliate_commission': '30',
    'affiliate_details': 'Earn 30% recurring commission for every paying user you refer. No cap, paid monthly via PayPal or bank transfer.',
    'changelog_v10': 'v10.0 -- May 2026: WhatsApp screenshot QC, GDPR auto-delete, Share report with client, API management panel',
    'changelog_v9': 'v9.0 -- April 2026: Token limit controls, Gift access, Content management, 5-country compliance',
    'changelog_v8': 'v8.0 -- March 2026: French termination fixes, QC certificate PDF, Report templates',
    'changelog_v7': 'v7.0 -- February 2026: Double check (Code+AI), Progress live log, Retest failed paths only',
    'changelog_v6': 'v6.0 -- January 2026: Multi-platform support (Decipher, Forsta, Qualtrics), SSO login',
}
...
def admin_content():
    ...
    # Group fields
    groups = {
        'Hero & Announcement': ['hero_heading','hero_subheading','hero_cta','announcement','tagline'],
        'Features': ['feature1_title','feature1_desc','feature2_title','feature2_desc','feature3_title','feature3_desc','feature4_title','feature4_desc','feature5_title','feature5_desc','feature6_title','feature6_desc'],
        ...
        'Docs': ['docs_intro'],
        'Site & Footer': ['site_name','tagline','footer_text','support_email','linkedin_url','twitter_url','youtube_url'],
        'Legal': ['privacy_policy','terms'],
    }

    alert = '<div style="background:#EAF3DE;border:0.5px solid #A5D6A7;border-radius:8px;padding:10px 14px;margin-bottom:16px;color:#27500A;font-size:13px">All content saved!</div>' if saved else ''

    form_html = ''
    for group, fields in groups.items():
        form_html += '<div style="background:white;border:0.5px solid #DDE1E7;border-radius:10px;padding:20px;margin-bottom:16px">'
        form_html += '<p style="font-size:14px;font-weight:600;color:#1A1A2E;margin-bottom:14px">'+group+'</p>'
        for field in fields:
            if field not in site_content:
                continue
            val = site_content.get(field, '')
            label = field.replace('_',' ').title()
            is_long = 'summary' in field or 'details' in field or 'heading' in field or 'policy' in field or 'terms' in field
            if is_long:
                input_html = '<textarea name="'+field+'" style="width:100%;padding:8px 10px;border:0.5px solid #DDE1E7;border-radius:7px;font-size:12px;color:#1A1A2E;min-height:70px;resize:vertical;outline:none">'+val+'</textarea>'
            else:
                input_html = '<input type="text" name="'+field+'" value="'+val+'" style="width:100%;padding:8px 10px;border:0.5px solid #DDE1E7;border-radius:7px;font-size:12px;color:#1A1A2E;outline:none">'
            form_html += '<div style="margin-bottom:12px"><p style="font-size:11px;font-weight:500;color:#6B7280;margin-bottom:4px">'+label+'</p>'+input_html+'</div>'
        form_html += '</div>'

    page = '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Content Management - Admin</title></head><body style="background:#F0F2F5;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;color:#1A1A2E">'
    page += '<div style="padding:24px;max-width:900px;margin:0 auto">'
    page += '<div style="display:flex;align-items:center;gap:14px;margin-bottom:20px"><a href="/admin" style="color:#6B7280;font-size:22px;text-decoration:none">&larr;</a><div><p style="font-size:20px;font-weight:600">Content Management</p><p style="font-size:12px;color:#6B7280">Landing page, blog, community, compare pages ka content yahan se edit karo</p></div></div>'
    page += alert
    page += '<form method="POST">'+form_html
    page += '<button type="submit" style="width:100%;background:#042C53;color:white;border:none;padding:14px;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;margin-top:8px">Save All Changes</button>'
...
def home_landing():
    ...
    # Feature cards
    feats = [
        ('ti-x-octagon','#E6F1FB','#185FA5',c['feature1_title'],c['feature1_desc']),
        ('ti-text-recognition','#E1F5EE','#0F6E56',c['feature2_title'],c['feature2_desc']),
        ('ti-camera','#EAF3DE','#3B6D11',c['feature3_title'],c['feature3_desc']),
        ('ti-world','#FAEEDA','#854F0B',c['feature4_title'],c['feature4_desc']),
        ('ti-shield-check','#FCEBEB','#A32D2D',c['feature5_title'],c['feature5_desc']),
        ('ti-refresh','#EEEDFE','#534AB7',c['feature6_title'],c['feature6_desc']),
    ]
    feat_cards = ''
    for icon,bg,col,title,desc in feats:
        feat_cards += ('<div style="background:white;border:0.5px solid #DDE1E7;border-radius:12px;padding:20px">'
            '<div style="width:40px;height:40px;border-radius:9px;background:'+bg+';display:flex;align-items:center;justify-content:center;margin-bottom:12px">'
            '<i class="ti '+icon+'" style="font-size:20px;color:'+col+'"></i></div>'
            '<p style="font-size:14px;font-weight:600;color:#1A1A2E;margin-bottom:6px">'+title+'</p>'
            '<p style="font-size:13px;color:#6B7280;line-height:1.6">'+desc+'</p></div>')
    ...
    blog_cards = ''
    for title,date,tag,summary in blog_items:
        tc = tag_colors.get(tag,'#F1EFE8|#444441').split('|')
        blog_cards += ('<a href="/blog" style="text-decoration:none">'
            '<div style="background:white;border:0.5px solid #DDE1E7;border-radius:12px;padding:20px">'
            '<span style="background:'+tc[0]+';color:'+tc[1]+';font-size:11px;padding:3px 10px;border-radius:20px;font-weight:500">'+tag+'</span>'
            '<p style="font-size:15px;font-weight:600;color:#1A1A2E;margin:10px 0 8px;line-height:1.4">'+title+'</p>'
            '<p style="font-size:13px;color:#6B7280;line-height:1.6;margin-bottom:12px">'+summary[:120]+'...</p>'
            '<p style="font-size:11px;color:#9CA3AF">'+date+'</p>'
            '</div></a>')

    # Testimonials
    tests = [
        ('ST','#E6F1FB','#0C447C','Sarah T.','QC Manager, UK','"8 hours of manual QC now takes 10 minutes. Caught a termination bug that would have killed our entire dataset."'),
        ('ML','#E1F5EE','#085041','Marie L.','Research Director, France','"French, Italian, Spanish -- handles all perfectly. Screenshot evidence is a total game changer for client reports."'),
        ('JM','#EEEDFE','#3C3489','James M.','Ops Lead, USA','"Saved our team 47 hours this month. ROI is insane. Nothing else comes close to this for survey professionals."'),
    ]
    test_cards = ''
    stars = '<i class="ti ti-star-filled" style="font-size:13px;color:#F59E0B"></i>' * 5
    for init,bg,col,name,role,quote in tests:
        test_cards += ('<div style="background:white;border:0.5px solid #DDE1E7;border-radius:12px;padding:22px">'
            '<div style="display:flex;gap:2px;margin-bottom:12px">'+stars+'</div>'
            '<p style="font-size:14px;color:#374151;line-height:1.7;margin-bottom:16px">'+quote+'</p>'
            '<div style="display:flex;align-items:center;gap:10px">'
            '<div style="width:36px;height:36px;border-radius:50%;background:'+bg+';color:'+col+';display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700">'+init+'</div>'
            '<div><p style="font-size:13px;font-weight:600;color:#1A1A2E">'+name+'</p>'
            '<p style="font-size:11px;color:#9CA3AF">'+role+'</p></div></div></div>')

    page = '<!DOCTYPE html><html lang="en"><head>'
    page += '<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">'
    page += '<title>'+c['site_name']+' - AI Survey QC Tool</title>'
    page += '<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@tabler/icons-webfont@3.19.0/dist/tabler-icons.min.css">'
    page += '<style>'
    page += '*{box-sizing:border-box;margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}'
    page += 'body{background:#F8F9FA;color:#1A1A2E}a{text-decoration:none}'
    page += '.pub-nav{background:white;border-bottom:0.5px solid #DDE1E7;padding:0 40px;display:flex;align-items:center;justify-content:space-between;height:60px;position:sticky;top:0;z-index:100}'
    page += '.nav-links{display:flex;align-items:center;gap:24px}'
    page += '.nav-link{font-size:13px;color:#6B7280}'
    page += '.nav-link:hover{color:#1A1A2E}'
    page += '.section{padding:70px 40px}'
    page += '.section-hd{text-align:center;max-width:700px;margin:0 auto 48px}'
    page += '.grid3{display:grid;grid-template-columns:repeat(3,1fr);gap:20px;max-width:1100px;margin:0 auto}'
    page += '.grid4{display:grid;grid-template-columns:repeat(4,1fr)}'
    page += '.stat-strip{border-top:0.5px solid #DDE1E7;border-bottom:0.5px solid #DDE1E7;background:white}'
    page += '.stat-item{padding:28px;text-align:center;border-right:0.5px solid #DDE1E7}'
    page += '.pcard{background:white;border:0.5px solid #DDE1E7;border-radius:14px;padding:28px}'
    page += '.pcard.pop{border:2px solid #185FA5;position:relative}'
    page += '.pfeat{font-size:13px;color:#6B7280;display:flex;align-items:center;gap:8px;margin-bottom:8px}'
    page += '.footer{background:#1A1A2E;padding:60px 40px 30px}'
    page += '.footer-grid{display:grid;grid-template-columns:2fr 1fr 1fr 1fr 1fr;gap:40px;margin-bottom:50px}'
    page += '.ft{font-size:13px;font-weight:600;color:white;margin-bottom:16px}'
    page += '.fl{display:block;font-size:13px;color:#9CA3AF;margin-bottom:10px}'
    page += '.fl:hover{color:white}'
    page += '.ann{background:#E6F1FB;padding:10px 40px;text-align:center;font-size:13px;color:#0C447C;font-weight:500}'
    page += '@media(max-width:768px){'
    page += '.pub-nav{padding:0 16px}.nav-links{display:none}'
    page += '.section{padding:50px 16px}'
    page += '.grid3{grid-template-columns:1fr}'
    page += '.grid4{grid-template-columns:repeat(2,1fr)}'
    page += '.footer-grid{grid-template-columns:1fr 1fr}'
    page += '}'
    page += '</style></head><body>'

    # Announcement
    page += '<div class="ann"><i class="ti ti-sparkles"></i> '+c['announcement']+' &nbsp;<a href="/signup" style="color:#042C53;font-weight:700;text-decoration:underline">Try free &rarr;</a></div>'

    # Nav
    page += '<nav class="pub-nav">'
    page += '<div style="display:flex;align-items:center;gap:10px"><div style="width:30px;height:30px;background:#042C53;border-radius:8px;display:flex;align-items:center;justify-content:center"><i class="ti ti-shield-check" style="font-size:15px;color:white"></i></div>'
    page += '<span style="font-size:16px;font-weight:700;color:#1A1A2E">'+c['site_name']+'</span></div>'
    page += '<div class="nav-links"><a href="#features" class="nav-link">Features</a><a href="#how" class="nav-link">How it works</a><a href="#pricing" class="nav-link">Pricing</a><a href="/blog" class="nav-link">Blog</a><a href="/docs" class="nav-link">Docs</a><a href="/community" class="nav-link">Community</a></div>'
    page += '<div style="display:flex;gap:10px"><a href="/login" style="font-size:13px;color:#6B7280;padding:8px 16px;border:0.5px solid #DDE1E7;border-radius:8px">Sign in</a>'
    page += '<a href="/signup" style="font-size:13px;color:white;background:#042C53;padding:8px 18px;border-radius:8px;font-weight:500">'+c['hero_cta']+'</a></div></nav>'

    # Hero
    page += '<div style="text-align:center;padding:80px 20px 60px;background:white">'
    page += '<div style="display:inline-flex;align-items:center;gap:6px;background:#E1F5EE;color:#085041;font-size:12px;padding:5px 16px;border-radius:20px;margin-bottom:20px;font-weight:500"><i class="ti ti-sparkles" style="font-size:12px"></i>'+c['tagline']+'</div>'
    page += '<h1 style="font-size:48px;font-weight:700;color:#1A1A2E;margin-bottom:16px;line-height:1.15;max-width:800px;margin-left:auto;margin-right:auto">'+c['hero_heading']+'</h1>'
    page += '<p style="font-size:18px;color:#6B7280;margin-bottom:32px;max-width:580px;margin-left:auto;margin-right:auto;line-height:1.7">'+c['hero_subheading']+'</p>'
    page += '<div style="display:flex;gap:14px;justify-content:center;margin-bottom:16px;flex-wrap:wrap">'
    page += '<a href="/signup" style="font-size:15px;color:white;background:#042C53;padding:14px 34px;border-radius:10px;font-weight:600">'+c['hero_cta']+'</a>'
    page += '<a href="#how" style="font-size:15px;color:#374151;border:0.5px solid #DDE1E7;padding:14px 28px;border-radius:10px;background:white">See how it works &rarr;</a></div>'
    page += '<p style="font-size:13px;color:#9CA3AF">No credit card &middot; 5 free reports/month forever</p>'
    page += '<div style="display:flex;gap:20px;justify-content:center;margin-top:24px;flex-wrap:wrap">'
    page += '<span style="font-size:13px;color:#6B7280;font-weight:500">&#10003; Confirmit</span>'
    page += '<span style="font-size:13px;color:#6B7280;font-weight:500">&#10003; Decipher</span>'
    page += '<span style="font-size:13px;color:#6B7280;font-weight:500">&#10003; Forsta</span>'
    page += '<span style="font-size:13px;color:#6B7280;font-weight:500">&#10003; Qualtrics</span>'
    page += '</div></div>'

    # Stats strip
    page += '<div class="stat-strip grid4">'
    page += '<div class="stat-item"><p style="font-size:32px;font-weight:700;color:#042C53">500+</p><p style="font-size:13px;color:#9CA3AF;margin-top:4px">QC professionals</p></div>'
    page += '<div class="stat-item"><p style="font-size:32px;font-weight:700;color:#042C53">8h</p><p style="font-size:13px;color:#9CA3AF;margin-top:4px">Saved per survey</p></div>'
    page += '<div class="stat-item"><p style="font-size:32px;font-weight:700;color:#042C53">99%</p><p style="font-size:13px;color:#9CA3AF;margin-top:4px">Accuracy rate</p></div>'
    page += '<div class="stat-item" style="border-right:none"><p style="font-size:32px;font-weight:700;color:#042C53">80+</p><p style="font-size:13px;color:#9CA3AF;margin-top:4px">Languages</p></div>'
    page += '</div>'

    # Features
    page += '<div class="section" id="features" style="background:#F8F9FA">'
    page += '<div class="section-hd"><p style="font-size:12px;color:#9CA3AF;text-transform:uppercase;letter-spacing:.12em;margin-bottom:10px">What we check automatically</p>'
    page += '<h2 style="font-size:34px;font-weight:700;color:#1A1A2E;margin-bottom:12px">Everything manual -- now automated</h2>'
    page += '<p style="font-size:16px;color:#6B7280;line-height:1.7">8 specialized QC checks run automatically in parallel. No config needed.</p></div>'
    page += '<div class="grid3">'+feat_cards+'</div></div>'

    # How it works
    page += '<div class="section" id="how" style="background:white">'
    page += '<div class="section-hd"><p style="font-size:12px;color:#9CA3AF;text-transform:uppercase;letter-spacing:.12em;margin-bottom:10px">How it works</p>'
    page += '<h2 style="font-size:34px;font-weight:700;color:#1A1A2E;margin-bottom:12px">3 steps to perfect QC</h2>'
    page += '<p style="font-size:16px;color:#6B7280;line-height:1.7">From upload to report in under 12 minutes.</p></div>'
    page += '<div class="grid3">'
    page += '<div style="background:#F8F9FA;border-radius:14px;padding:28px;text-align:center"><div style="width:44px;height:44px;border-radius:50%;background:#042C53;color:white;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;margin:0 auto 16px">1</div><p style="font-size:16px;font-weight:600;color:#1A1A2E;margin-bottom:8px">Upload doc + URL</p><p style="font-size:14px;color:#6B7280;line-height:1.6">Upload your screener .docx and paste the live survey URL. Takes 30 seconds.</p></div>'
    page += '<div style="background:#E6F1FB;border-radius:14px;padding:28px;text-align:center;border:0.5px solid #B5D4F4"><div style="width:44px;height:44px;border-radius:50%;background:#378ADD;color:white;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;margin:0 auto 16px">2</div><p style="font-size:16px;font-weight:600;color:#1A1A2E;margin-bottom:8px">AI tests everything</p><p style="font-size:14px;color:#6B7280;line-height:1.6">AI crawls every path, runs all 8 checks, captures screenshots. Double verified.</p></div>'
    page += '<div style="background:#F8F9FA;border-radius:14px;padding:28px;text-align:center"><div style="width:44px;height:44px;border-radius:50%;background:#042C53;color:white;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;margin:0 auto 16px">3</div><p style="font-size:16px;font-weight:600;color:#1A1A2E;margin-bottom:8px">Get Word report</p><p style="font-size:14px;color:#6B7280;line-height:1.6">Download detailed report with all issues, fix suggestions, screenshots, and QC certificate.</p></div>'
    page += '</div></div>'

    # Testimonials
    page += '<div class="section" style="background:#F8F9FA">'
    page += '<div class="section-hd"><h2 style="font-size:34px;font-weight:700;color:#1A1A2E;margin-bottom:12px">Trusted by QC professionals</h2>'
    page += '<p style="font-size:16px;color:#6B7280">From market research agencies to in-house teams worldwide.</p></div>'
    page += '<div class="grid3">'+test_cards+'</div></div>'

    # Pricing
    page += '<div class="section" id="pricing" style="background:white">'
    page += '<div class="section-hd"><p style="font-size:12px;color:#9CA3AF;text-transform:uppercase;letter-spacing:.12em;margin-bottom:10px">Simple pricing</p>'
    page += '<h2 style="font-size:34px;font-weight:700;color:#1A1A2E;margin-bottom:12px">Start free, upgrade anytime</h2>'
    page += '<p style="font-size:16px;color:#6B7280">No contracts &middot; Cancel anytime &middot; 14-day free trial on paid plans</p></div>'
    page += '<div class="grid3">'
    # Free
    page += '<div class="pcard"><p style="font-size:15px;font-weight:600;color:#1A1A2E">Free</p><p style="font-size:36px;font-weight:700;margin:10px 0 4px">$0<span style="font-size:15px;font-weight:400;color:#9CA3AF">/mo</span></p><p style="font-size:13px;color:#9CA3AF;margin-bottom:20px">5 reports/month forever</p>'
    page += '<div class="pfeat"><i class="ti ti-check" style="color:#1D9E75;font-size:16px"></i>All 8 QC checks</div>'
    page += '<div class="pfeat"><i class="ti ti-check" style="color:#1D9E75;font-size:16px"></i>Word report download</div>'
    page += '<div class="pfeat"><i class="ti ti-check" style="color:#1D9E75;font-size:16px"></i>Any language</div>'
    page += '<div class="pfeat"><i class="ti ti-check" style="color:#1D9E75;font-size:16px"></i>20K tokens/report</div>'
    page += '<a href="/signup" style="display:block;text-align:center;border:0.5px solid #DDE1E7;padding:11px;border-radius:9px;font-size:14px;color:#374151;margin-top:20px;font-weight:500">Get started free</a></div>'
    # Pro
    page += '<div class="pcard pop"><span style="background:#042C53;color:white;font-size:11px;padding:4px 14px;border-radius:20px;position:absolute;top:-14px;right:20px;font-weight:500">Most popular</span>'
    page += '<p style="font-size:15px;font-weight:600;color:#1A1A2E">Pro</p><p style="font-size:36px;font-weight:700;margin:10px 0 4px">$29<span style="font-size:15px;font-weight:400;color:#9CA3AF">/mo</span></p><p style="font-size:13px;color:#9CA3AF;margin-bottom:20px">50 reports/month</p>'
    page += '<div class="pfeat"><i class="ti ti-check" style="color:#1D9E75;font-size:16px"></i>Everything in Free</div>'
    page += '<div class="pfeat"><i class="ti ti-check" style="color:#1D9E75;font-size:16px"></i>AI auto tester</div>'
    page += '<div class="pfeat"><i class="ti ti-check" style="color:#1D9E75;font-size:16px"></i>Auto screenshots</div>'
    page += '<div class="pfeat"><i class="ti ti-check" style="color:#1D9E75;font-size:16px"></i>Share report + client comments</div>'
    page += '<div class="pfeat"><i class="ti ti-check" style="color:#1D9E75;font-size:16px"></i>100K tokens/report</div>'
    page += '<a href="/signup" style="display:block;text-align:center;background:#042C53;color:white;padding:11px;border-radius:9px;font-size:14px;font-weight:600;margin-top:20px">Start Pro trial &rarr;</a></div>'
    # Business
    page += '<div class="pcard"><p style="font-size:15px;font-weight:600;color:#1A1A2E">Business</p><p style="font-size:36px;font-weight:700;margin:10px 0 4px">$99<span style="font-size:15px;font-weight:400;color:#9CA3AF">/mo</span></p><p style="font-size:13px;color:#9CA3AF;margin-bottom:20px">Unlimited reports</p>'
    page += '<div class="pfeat"><i class="ti ti-check" style="color:#1D9E75;font-size:16px"></i>Everything in Pro</div>'
    page += '<div class="pfeat"><i class="ti ti-check" style="color:#1D9E75;font-size:16px"></i>Team collaboration</div>'
    page += '<div class="pfeat"><i class="ti ti-check" style="color:#1D9E75;font-size:16px"></i>Own API key</div>'
    page += '<div class="pfeat"><i class="ti ti-check" style="color:#1D9E75;font-size:16px"></i>White label reports</div>'
    page += '<div class="pfeat"><i class="ti ti-check" style="color:#1D9E75;font-size:16px"></i>150K tokens/report</div>'
    page += '<a href="/signup" style="display:block;text-align:center;border:0.5px solid #DDE1E7;padding:11px;border-radius:9px;font-size:14px;color:#374151;margin-top:20px;font-weight:500">Get Business</a></div>'
    page += '</div></div>'

    # Blog
    page += '<div class="section" style="background:#F8F9FA">'
    page += '<div class="section-hd"><p style="font-size:12px;color:#9CA3AF;text-transform:uppercase;letter-spacing:.12em;margin-bottom:10px">From the blog</p>'
    page += '<h2 style="font-size:34px;font-weight:700;color:#1A1A2E;margin-bottom:12px">QC insights & guides</h2>'
    page += '<p style="font-size:16px;color:#6B7280">Best practices, platform guides, and industry news.</p></div>'
    page += '<div class="grid3">'+blog_cards+'</div>'
    page += '<div style="text-align:center;margin-top:32px"><a href="/blog" style="font-size:14px;color:#042C53;border:0.5px solid #042C53;padding:11px 28px;border-radius:9px;font-weight:500">Read all articles &rarr;</a></div>'
    page += '</div>'

    # CTA Banner
    page += '<div style="background:#042C53;padding:70px 40px;text-align:center">'
    page += '<h2 style="font-size:36px;font-weight:700;color:white;margin-bottom:12px">Save 8 hours per survey starting today</h2>'
    page += '<p style="font-size:16px;color:#85B7EB;margin-bottom:32px">Join 500+ QC professionals worldwide &middot; Free forever &middot; No credit card needed</p>'
    page += '<a href="/signup" style="background:white;color:#042C53;font-size:15px;padding:15px 40px;border-radius:10px;font-weight:700">Start free -- no card required</a></div>'

    # Footer
    page += '<footer class="footer">'
    page += '<div class="footer-grid">'
    # Brand col
    page += '<div><div style="display:flex;align-items:center;gap:9px;margin-bottom:16px"><div style="width:30px;height:30px;background:#378ADD;border-radius:7px;display:flex;align-items:center;justify-content:center"><i class="ti ti-shield-check" style="font-size:15px;color:white"></i></div><span style="font-size:16px;font-weight:700;color:white">'+c['site_name']+'</span></div>'
    page += '<p style="font-size:13px;color:#6B7280;line-height:1.7;margin-bottom:12px">The worlds first AI-powered survey QC tool. Built for QC professionals worldwide.</p>'
    page += '<p style="font-size:13px;color:#9CA3AF;margin-bottom:4px">&#169; 2026 '+c['site_name']+'</p>'
    page += '<a href="mailto:'+c['support_email']+'" style="font-size:13px;color:#6B7280">'+c['support_email']+'</a>'
    page += '<div style="display:flex;gap:10px;margin-top:16px"><a href="'+c['linkedin_url']+'" style="width:32px;height:32px;background:rgba(255,255,255,.1);border-radius:6px;display:flex;align-items:center;justify-content:center;color:#9CA3AF"><i class="ti ti-brand-linkedin" style="font-size:16px"></i></a>'
    page += '<a href="'+c['twitter_url']+'" style="width:32px;height:32px;background:rgba(255,255,255,.1);border-radius:6px;display:flex;align-items:center;justify-content:center;color:#9CA3AF"><i class="ti ti-brand-twitter" style="font-size:16px"></i></a></div></div>'
    # Resources
    page += '<div><p class="ft">Resources</p><a href="#pricing" class="fl">Pricing</a><a href="/docs" class="fl">Documentation</a><a href="/blog" class="fl">Blog</a><a href="/changelog" class="fl">Changelog</a><a href="#features" class="fl">Features</a></div>'
    # Compare
    page += '<div><p class="ft">Compare</p><a href="/compare/chatgpt" class="fl">vs ChatGPT</a><a href="/compare/excel" class="fl">vs Excel QC</a><a href="/compare/manual" class="fl">vs Manual Testing</a></div>'
    # Community
    page += '<div><p class="ft">Community</p><a href="/community" class="fl">Community hub</a><a href="/affiliate" class="fl">Affiliate program</a><a href="/blog" class="fl">Newsletter</a></div>'
    # Account + Legal
    page += '<div><p class="ft">Account</p><a href="/login" class="fl">Sign in</a><a href="/signup" class="fl">Sign up</a><a href="/dashboard" class="fl">Dashboard</a>'
    page += '<p class="ft" style="margin-top:20px">Legal</p><a href="/privacy-policy" class="fl">Privacy policy</a><a href="/terms" class="fl">Terms of service</a></div>'
    page += '</div>'
    # Footer bottom
    page += '<div style="border-top:0.5px solid rgba(255,255,255,.1);padding-top:24px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px">'
    page += '<p style="font-size:12px;color:#6B7280">'+c['footer_text']+'</p>'
    page += '<div style="display:flex;gap:16px"><a href="/privacy-policy" style="font-size:12px;color:#6B7280">Privacy</a><a href="/terms" style="font-size:12px;color:#6B7280">Terms</a><a href="mailto:'+c['support_email']+'" style="font-size:12px;color:#6B7280">Contact</a></div>'
    page += '</div></footer>'
    page += '</body></html>'
    return page
