...
def _confirmit_project_get_text(question_el) -> str:
    ...
    for ft in form_texts_el:
        ...
        if text_el is not None and text_el.text and text_el.text.strip():
            clean = re.sub(r'<[^>]+>', '', text_el.text).strip()
            if clean:
                texts[lang] = clean
    ...
    if texts_el is not None:
        ...
        for text_el in texts_el:
            if text_el.tag.split('}')[-1].lower() == 'text':
                ...
                val  = re.sub(r'<[^>]+>', '', text_el.text or '').strip()
                if val:
                    texts[lang] = val
        for pref in ('2057', '1033', 'ENG', 'EN', ''):
            if pref in texts:
...
def _parse_confirmit_project_xml(root) -> tuple:
    ...
    for el in root.iter():
        tag = el.tag.split('}')[-1]
        if tag not in _CONFIRMIT_PROJECT_Q_TAGS:
            continue
        ...
        # Grid/Scale types: also try <Scales><Scale> for column headers when rows are empty
        ...
        # Routing: prefer <QuestionMaskPredicate>, fall back to <Predicates>/<Expression>
        routing = None
        ...
        if triggers:
            llm_count += 1
            log.info("_parse_confirmit_project_xml: LLM fallback QID=%s fields=%s", qid, triggers)
            q = _llm_enrich(qid, raw_block, q, triggers)
